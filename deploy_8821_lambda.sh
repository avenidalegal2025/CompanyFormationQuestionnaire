...
# Deploy 8821 Lambda Function
# This script packages and deploys the 8821 Lambda function to AWS
...
FUNCTION_NAME="${LAMBDA_8821_NAME:-Fill8821Lambda-arm64}"
REGION="${AWS_REGION:-us-west-1}"
RUNTIME="python3.11"
HANDLER="lambda_function.lambda_handler"
TIMEOUT=300
MEMORY_SIZE=512
ROLE_NAME="${LAMBDA_ROLE_NAME:-lambda-8821-role}"

echo "🚀 Deploying 8821 Lambda Function..."
...
# Copy Lambda function and rename to lambda_function.py
cp lambda-functions/8821_lambda_s3_complete.py "$TEMP_DIR/lambda_function.py"
...
pip install reportlab PyPDF2 typing_extensions boto3 -t . --quiet
...
# Create zip file
ZIP_FILE="/tmp/8821-lambda-deployment.zip"
...
EOF
        ...
        aws iam create-role \
            ...
            --description "Role for 8821 Lambda function" \
            --output json > /dev/null
...
EOF
    ...
    aws lambda create-function \
        ...
        --memory-size "$MEMORY_SIZE" \
        --region "$REGION" \
        --description "Form 8821 Tax Information Authorization PDF Generator" \
        --environment "Variables={$ENV_VARS}" \
        --output json | jq -r '.FunctionArn'
