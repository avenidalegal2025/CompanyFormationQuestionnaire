...
def map_data_to_ss4_fields(form_data):
    ...
    mapped_data = {
        ...
        "11": to_upper(business_purpose or "General business operations"),  # Principal activity - ALL CAPS
        "12": "",  # Principal activity code (NAICS code - not provided)
        "13": {
            "Agricultural": "0",
            "Household": "0",
            "Other": "0"
        },
        "15": "N/A",  # First date wages paid (N/A if no employees)
        ...
        "Designee Fax": "",
        "Applicant Phone": form_data.get("applicantPhone", ""),
        "Applicant Fax": "",
        "Signature Name": to_upper(responsible_name),  # ALL CAPS
        "Checks": {}
    }
    ...
    if is_llc:
        ...
        # Corporation checkbox - 25 pixels below Partnership position (only for sole proprietor)
        if is_sole_proprietor:
    ...
    # If sole proprietor has SSN, add it to the field next to the checkbox
    if is_sole_proprietor and sole_proprietor_ssn:
        mapped_data["9a_sole_ssn"] = sole_proprietor_ssn
    ...
    # Usually left blank for service businesses
    
    # Line 17: Has applicant applied for EIN before? (default to No)
    # No checkbox needed if answer is No
    
    # Line 18: Third Party Designee (Checkbox - "Yes" if designee is authorized)
    # Default to Yes since we're providing Avenida Legal as designee
    mapped_data["Checks"]["18_yes"] = [401, 159]  # Approximate position for "Yes"
