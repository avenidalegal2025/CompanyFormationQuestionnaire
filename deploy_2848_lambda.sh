...
# Deploy 2848 Lambda Function
# This script packages and deploys the 2848 Lambda function to AWS
...
TIMEOUT=300
MEMORY_SIZE=512
ROLE_NAME="${LAMBDA_ROLE_NAME:-lambda-2848-role}"

echo "🚀 Deploying 2848 Lambda Function..."
...
cp lambda-functions/2848_lambda_s3.py "$TEMP_DIR/lambda_function.py"
...
# Create zip file
ZIP_FILE="/tmp/2848-lambda-deployment.zip"
...
EOF
        ...
        aws iam create-role \
            ...
            --description "Role for 2848 Lambda function" \
            --output json > /dev/null
...
EOF
    ...
    aws lambda create-function \
        ...
        --memory-size "$MEMORY_SIZE" \
        --region "$REGION" \
        --description "Form 2848 Power of Attorney PDF Generator" \
        --environment "Variables={$ENV_VARS}" \
        --output json | jq -r '.FunctionArn'
