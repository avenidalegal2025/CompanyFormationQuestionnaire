/**
 * DEMO LLC: AVENIDA CONSULTING LLC (Florida)
 *
 ...
 *   - Tax Partner: Antonio Regojo
 *   - Non-compete: No
 *   - Non-solicitation: Yes
 *   - Confidentiality: Yes
 *   - Non-disparagement: Yes (always included in template)
 *   - Bank: Dos firmantes
 ...
 *   - Tag/drag: No
 *
 * Auth0: test+demo_llc_TIMESTAMP@gmail.com / DemoPass2026!
 * Stripe: 4242 4242 4242 4242
 *
 * After payment: wait for documents, screenshot dashboard.
 * Screenshots saved to C:/Users/neotr/Downloads/demo-companies/llc/
 */
...
const DIR = join(process.env.USERPROFILE || '.', 'Downloads', 'demo-companies', 'llc');
mkdirSync(DIR, { recursive: true });

const TIMESTAMP = Date.now();
const EMAIL = `test+demo_llc_${TIMESTAMP}@gmail.com`;
...
async function fillStep1(page) {
  ...
  // Select suffix and state from dropdowns
...
async function main() {
  console.log('=== DEMO LLC: AVENIDA CONSULTING ===');
  console.log('Dir: ' + DIR);
  ...
  try {
    ...
    // Capital contributions: $60,000 / $40,000
    ...
    // Non-solicitation: Yes (visible because non-compete is No)
    ...
    // Non-disparagement: always included in template, no toggle
    ...
    if (stripeUrl.includes('stripe') || stripeUrl.includes('checkout.stripe')) {
      ...
      // Dashboard screenshots - take multiple viewport positions
      await page.evaluate(() => window.scrollTo(0, 0));
      await shot(page, 'dashboard_top');
      await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight / 2));
      await shot(page, 'dashboard_middle');
      await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
