...
export async function POST(request: NextRequest) {
  try {
    ...
    // Define the most common domain extensions
    ...
    // Create mock results with real pricing
    const results = domainsToCheck.map((domainName: string) => {
      ...
      // Get real pricing from Namecheap
      const realPricing = pricingData[domainName] || {
        price: 12.99,
        renewal_price: 14.99,
        currency: 'USD'
      };
      ...
      // Mock availability (simulate some domains as available)
